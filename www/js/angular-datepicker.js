...
(function withAngular(angular, navigator) {
  ...
  var A_DAY_IN_MILLISECONDS = 86400000
    ...
    , datepickerDirective = function datepickerDirective($window, $compile, $locale, $filter, $interpolate, Appointment, CityBranchId, $ionicLoading, $rootScope) {

      var linkingFunction = function linkingFunction($scope, element, attr) {

        //get child input

        console.log("date min limit", $scope.dateMinLimit)
        var selector = attr.selector
          ...
          , setDaysInMonth = function setDaysInMonth(month, year) {
            console.log("in set days", month,year)
            var i
            ...
            for (i = 1; i <= limitDate; i += 1) {
              // var abcDate = new Date(year,4,i).getDay();
              // if (abcDate == 5) {
              //   console.log("friday found");
              //   dateDisabledDates.push( new Date(year,month,i))
              // }
              $scope.days.push(i);
            }
            //dateDisabledDates.push(new Date('2017-04-14'))
        ...
        $scope.prevMonth = function managePrevMonth() {
          ...
          if ($scope.dateMinLimit) {
              console.log("in min date if" + $scope.year + '/' + $scope.monthNumber + '/' + $scope.days[$scope.days.length - 1])
        ...
        $scope.isSelectableMinDate = function isSelectableMinDate(aDate) {
          //if current date
          // console.log("selectable min adate", new Date(aDate).getTime() )
          // var x = new Date("2016-12-31T16:00:00").getTime();
          // console.log("selectable min dateMinLimit",x)
          if (!!$scope.dateMinLimit &&
             !!new Date($scope.dateMinLimit) &&
             new Date(aDate).getTime() < new Date($scope.dateMinLimit).getTime()) {
            //new Date(aDate).getTime() < x) {
        ...
        console.log("month number is", $scope.monthNumber)
        setDaysInMonth($scope.monthNumber, $scope.year);
  ...
  angular.module('720kb.datepicker', [])
               .directive('datepicker', ['$window', '$compile', '$locale', '$filter', '$interpolate', 'Appointment', 'CityBranchId', '$ionicLoading', '$rootScope', datepickerDirective]);
}(angular, navigator));
